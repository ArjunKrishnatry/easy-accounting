...
export default function SideTable({ uploadedFiles, onFileSelect, selectedFileId, onRefresh, onDelete }: SideTableProps) {
            ...
            >
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                        ...
                        <p className="font-semibold text-slate-900">{file.totalRecords}</p>
                    </div>
                    <div>
                ...
                {renamingFile === file.id && (
                    <div className="mt-3 pt-3 border-t border-slate-200" onClick={(e) => e.stopPropagation()}>
                        <input
                            ...
                            className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent mb-2"
                ...
                {showFolderDropdown === file.id && (
                    <div className="mt-3 pt-3 border-t border-slate-200" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-slate-700">Move to folder:</span>
                            <button
                        ...
                        {availableFolders.length === 0 ? (
                            <p className="text-sm text-slate-500 text-center py-2">No folders available</p>
                        ) : (
                            <div className="space-y-1">
                                {availableFolders.map(folder => (
                                    <button
                                        ...
                                        className="w-full text-left px-3 py-2 text-sm bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg transition-colors flex items-center gap-2"
                                    >
                                        <svg className="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                                        </svg>
    ...
    const renderFolder = (folder: FolderRecord) => (
        <div key={folder.id} className="bg-white border-2 border-slate-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div
                className={`flex items-center justify-between p-4 cursor-pointer transition-colors ${
                    expandedFolders.has(folder.id) ? 'bg-primary-50 border-b-2 border-primary-200' : 'hover:bg-slate-50'
                }`}
                onClick={() => toggleFolder(folder.id)}
            >
                <div className="flex items-center gap-3 flex-1">
                    <svg className={`w-5 h-5 text-slate-400 transition-transform ${expandedFolders.has(folder.id) ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                    <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                    </svg>
                    <h3 className="font-semibold text-slate-900 text-sm">{folder.name}</h3>
            ...
            {expandedFolders.has(folder.id) && (
                <div className="p-4 space-y-3 bg-slate-50">
                    {folder.files.length === 0 ? (
                        <p className="text-sm text-slate-500 text-center py-4">No files in this folder</p>
                    ) : (
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Create Folder Form */}
            {showCreateFolder && (
                <div className="bg-white border-2 border-primary-300 rounded-lg p-4 shadow-sm">
                    <input
                        type="text"
                        ...
                        className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent mb-3"
                        autoFocus
                    />
                    <div className="flex gap-2">
                        <button
